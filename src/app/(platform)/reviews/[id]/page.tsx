...
export default function ReviewSummaryPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  return (
    <div>
      <Link href="/recommendations" className="inline-flex items-center text-sm font-semibold text-slate-500 hover:text-brand-600">← Back to AI picks</Link>
      <p className="mt-4 text-sm font-semibold text-brand-600">REVIEW INSIGHTS</p>
      <h1 className="mt-1 text-3xl font-bold">What buyers are saying</h1>
      ...
      {summary && (
        <>
          <div className="mt-6 grid gap-4 sm:grid-cols-3">
            <div className="card p-5"><p className="text-xs font-semibold text-slate-500">Reviews analyzed</p><p className="mt-1 text-2xl font-bold">{summary.reviews_analyzed.toLocaleString()}</p></div>
            <div className="card p-5"><p className="text-xs font-semibold text-slate-500">Average rating</p><p className="mt-1 text-2xl font-bold">★ {summary.rating_breakdown.average_rating ?? "n/a"}</p></div>
            <div className={`card p-5 ${sentimentTone[summary.overall_sentiment.label] ?? ""}`}><p className="text-xs font-semibold opacity-80">Overall sentiment</p><p className="mt-1 text-2xl font-bold capitalize">{summary.overall_sentiment.label.replace(/_/g, " ")}</p></div>
          </div>

          <div className="card mt-4 p-5">
            <div className="flex flex-wrap items-center justify-between gap-2">
              <h2 className="font-bold">Recommendation</h2>
              <span className="rounded-full bg-brand-50 px-3 py-1 text-xs font-bold text-brand-700">{verdictLabel[summary.recommendation.verdict] ?? summary.recommendation.verdict}</span>
            </div>
          ...
          <div className="card mt-4 p-5">
            <h2 className="font-bold">Common complaints</h2>
            <ul className="mt-2 space-y-2 text-sm text-slate-600">
              {summary.common_complaints.map((item) => (
                <li key={item.complaint} className="flex items-center justify-between gap-4">
                  ...
                  <span className="whitespace-nowrap rounded-full bg-slate-100 px-2 py-0.5 text-xs font-semibold text-slate-500">{item.mention_count}x</span>
                </li>
              ))}
          ...
          {summary.top_keywords.length > 0 && (
            <div className="card mt-4 p-5">
              <h2 className="font-bold">Frequently mentioned</h2>
              <div className="mt-2 flex flex-wrap gap-2">{summary.top_keywords.map((keyword) => <span key={keyword.keyword} className="rounded-full bg-slate-100 px-3 py-1 text-xs text-slate-600">{keyword.keyword} · {keyword.mentions}</span>)}</div>
            </div>
          )}
        </>
      )}
