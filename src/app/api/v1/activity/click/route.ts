...
/** Records a shopper viewing/clicking a product - the lowest-intent personalization signal. */
export async function POST(request: Request) {
  try {
    const authorization = request.headers.get("authorization");
    const input = inputSchema.parse(await request.json());
    const backendResponse = await fetch(`${env.BACKEND_API_URL}/activity/click`, {
      method: "POST",
